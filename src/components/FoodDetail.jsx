...
function FoodDetail({ selectedRestaurant }) {
      ...
      if (selectedRestaurant?.restaurants_id) {
        try {
          const response = await fetch(
            `https://makter-testbackend.fly.dev/api/v1/restaurants/${selectedRestaurant.restaurants_id}/reviews`
          );
          if (response.ok) {
  ...
  const handleReviewSubmit = async (reviewText) => {
    try {
      const response = await fetch(
        `https://makter-testbackend.fly.dev/api/v1/reviews`,
        {
          method: "POST",
          headers: {
  ...
  const handleReviewDelete = async (reviewId) => {
    const response = await fetch(
      `https://makter-testbackend.fly.dev/api/v1/reviews/${reviewId}`,
      {
        method: "DELETE",
      }
    );
